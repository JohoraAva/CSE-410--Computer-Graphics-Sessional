...
class Matrix
{
    ...
    Point rodriguesFunction(Point axis, Point p,double radian)
    {
        Point res;
        res=axis*cos(radian)+p*(p.dot(axis))*(1-cos(radian))+(p*axis)*sin(radian);
        return res;
    }
    Matrix rotate(double angle, Point p)
    {
        ...
        Point c1= rodriguesFunction(Point(1,0,0),p,radian);
        Point c2= rodriguesFunction(Point(0,1,0),p,radian);
        Point c3= rodriguesFunction(Point(0,0,1),p,radian);

        res.valueAssign(c1,c2,c3);
        return res;
    ...
    Point operator*(Point p)
    {
        ...
        double output[4]={0.0};
        double temp[4]={p.x, p.y, p.z,1};

        for(int i=0; i<4; i++)
        {
            for(int j=0; j<4; j++)
            {
                output[i] += mat[i][j] * temp[j];
            }
        }
...
void stage4()
{
    ...
    while(fin>>p1.x>>p1.y>>p1.z)
    {
     ...
     //   cout<<"minY: "<<minY<<" maxY: "<<maxY<<" minX: "<<minX<<" maxX: "<<maxX<<endl;
        ...
        {
            for(double j=minX+0.003;j<maxX;j+=dx)
            {
